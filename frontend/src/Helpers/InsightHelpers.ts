...
export function transformInsightToMonthlyBreakdown(insight: Insight) {
  ...
  const categoryBreakdown = insight.categories.reduce((acc: Record<string, string>, curr) => {
...
export function getTopCategories(insight: Insight, x?: number) {
  const sortedCategories = insight.categories.toSorted((a, b) => b.amount - a.amount).map(prop('category'))
