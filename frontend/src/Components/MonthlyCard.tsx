...
export const MonthlyCard = ({ insights }: MonthlyCardProps) => {
  ...
  const colorMapForCategories = getRandomColorMap([
    ...allCategories,
    'negatives',
  ])
      ...
      <Card.Body>
        ...
        >
          <BarChart width={360} height={360} data={monthlyData}>
            ...
            <YAxis />
            <Tooltip />
            {[...allCategories, 'negatives'].map(category => {
              return (
                <Bar
                  key={category}
                  dataKey={category}
                  stackId={category === 'negatives' ? 'a' : 'b'}
              ...
              <Table>
                ...
                <tbody>
                  <tr key="total">
                    <td>Total</td>
                    <td>
                      {Math.round(monthlyData[activeIndex].amount * 100) / 100}
                    </td>
                  </tr>
