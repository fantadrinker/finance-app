...
export function UploadPreviewModal({
  ...
  const processActivitiesJSON = async () => {
    ...
    const selectedActivities = activities.filter(({ id }) =>
      selectedIds.has(id)
    )
    try {
  ...
  return (
    <Modal show={!!show} onHide={closeModal} size="lg">
      ...
      <Modal.Footer>
        <span>{selectedIds.size} Activities Selected</span>
        <Button onClick={closeModal}>Close</Button>
        <Button
          onClick={processActivitiesJSON}
          disabled={!!processingFile && selectedIds.size > 0}
        >
          Process Selected Activities
        </Button>
        <Button onClick={downloadAsCsv} disabled={!!processingFile}>
          Download as CSV
        </Button>
      </Modal.Footer>
    </Modal>
